...
** [5.13](#513) | [5.12](#512-p1) | [5.11](#511-p3) | [5.10](#510-p1) | [5.9](#59-p3) | [5.8](#58-p4) | [5.7](#57-p4) | [5.6](#56-p4) | [5.5](#55) | [5.4](#54-p3) | [5.3](#53-p5) | [5.2](#52-p7) **

---

[Halon](https://halon.io/) is a fast, flexible and powerful email infrastructure software (MTA) for demanding uses such as large-scale email services. 
It's available as a Linux package for various LTS distributions, as well as integrated (VM) packages for various hypervisors. 
...
## 5.13
Released 2023-08-15
- **`Imp`** Updated version of eleven eXpurgate anti-spam daemon (4.47.0)
- **`Imp`** Added selector field to DMARC reporting
- **`Imp`** Increased SA custom rules limit
- **`Imp`** Improved query builder for metadata fields in queue listing
- **`Imp`** Based on FreeBSD 13.2, and latest quarterly packages
- **`Bug`** Fix issue with SpamHaus DQS
- **`Bug`** Fix issue with memory_store HSL function
- **`Dep`** Removed deprecated ClamAV (ScanCLAM() is now a no-op)
...
- **`Imp`** MTA improvements
  ...
  - Added `queue.loader.active` counter to hold total messages currently loaded
...
- **`Imp`** Native (C ABI) plugin improvements
  ...
  - Create MailMessage and File classes from strings with [`HalonMTA_hsl_value_set()`](https://docs.halon.io/manual/archive/5.9-stable/plugins_native.html#c.HalonMTA_hsl_value_set)
  - Detect MailMessage type with [`HalonMTA_hsl_value_type()`](https://docs.halon.io/manual/archive/5.9-stable/plugins_native.html#c.HalonMTA_hsl_value_type)
...
- **`Imp`** Added decode option to [MIMEPart.getHeader](https://docs.halon.io/hsl/archive/5.6-stable/functions.html#MIMEPart.getHeader) functions
